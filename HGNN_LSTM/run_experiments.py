...
# Define edges
# Meteo -> Hydro connections
meteo_to_hydro_edges = torch.tensor([
    [0, 0, 0, 2, 2, 2, 2, 3, 4, 4, 4, 4, 4],  # Source: Meteo station indices
    [1, 3, 4, 7, 4, 1, 6, 2, 0, 1, 4, 6, 5]   # Target: Hydro station indices
])
# Hydro -> Hydro connections
hydro_to_hydro_edges = torch.tensor([
    [0, 1, 2, 3, 5, 6, 7],  # Source: Hydro station indices
    [6, 4, 6, 1, 6, 1, 6]   # Target: Hydro station indices
])
config["meteo_edge_index"] = meteo_to_hydro_edges
config["hydro_edge_index"] = hydro_to_hydro_edges

total = len(lr_list) * len(bs_list)
path = os.path.join("HGNN-LSTM", f"experiments_HGNN-LSTM_seed{seed}")
