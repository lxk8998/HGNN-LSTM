...
meteo_to_hydro_edges = torch.tensor([
    [0, 0, 0, 2, 2, 2, 2, 3, 4, 4, 4, 4, 4],  # Source: Meteo and Hydro station indices
...
path = os.path.join("temporal_model", f"experiments_temporal_model_seed{seed}")
for lr, bs in tqdm(product(lr_list, bs_list), total=total, desc="Task"):
    config["lr"] = lr
    config["batch_size"] = bs
    exp_name = f"lr{lr}_bs{bs}_ep{config['epochs']}_hidden_dim{config['hidden_dim']}_seed{config['seed']}"
    ...
    res = temporal_model_main.train_and_evaluate(
        config=config,
        futurelen=futurelen,
        exp_dir=exp_dir,
        ckpt_path=ckpt_dir,
