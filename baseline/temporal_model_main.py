...
import os
import models
from utils.concat_features import concat_hydro_meteo_features
...
def train_and_evaluate(config, futurelen, exp_dir, ckpt_path, ckpt_path_best, plot_dir, node_dir):
    ...
    feature_dim = meteo_feature_dim + hydro_feature_dim
    ...
    hydro_all_train, train_label, hydro_all_valid, valid_label, hydro_all_test, test_label = split_hydro.stack_hydro(df_hydro,
                                                                            ...
                                                                            hydro_feature_dim)
    # concat meteorological features
    all_data_train, all_data_valid, all_data_test = concat_hydro_meteo_features(
    hydro_all_train,
    hydro_all_valid,
    hydro_all_test,
    meteo_all_train,
    ...
    train_dataset = TensorDataset(all_data_train, train_label)
    ...
    valid_dataset = TensorDataset(all_data_valid, valid_label)
    valid_loader = DataLoader(valid_dataset, batch_size=batch_size, shuffle=False, drop_last=False, num_workers=0)
    test_dataset = TensorDataset(all_data_test, test_label)
    test_loader = DataLoader(test_dataset, batch_size=batch_size, shuffle=False, drop_last=False, num_workers=0)

    model = models.GRU(hidden_dim, feature_dim, futurelen, num_hydro_stations)
    # model = models.LSTM(hidden_dim, feature_dim, futurelen, num_hydro_stations)
    # model = models.Transformer(hidden_dim, feature_dim, futurelen, num_hydro_stations, nhead, num_layers)
    model.to(device)
